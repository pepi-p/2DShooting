...
namespace Enemy
{
    public class Normal : EnemyAction
    {
        private Vector3 _startPos;
        private Vector3 _entryPos;
        
        public Normal(Player player, EnemyBulletSpawner bulletSpawner, Vector3 startPos) : base(player, bulletSpawner)
        {
        ...
        private async UniTask Main(Transform transform, CancellationToken token)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                for (int j = -1; j <= 1; j++) _bulletSpawner.Spawn(normalBullet, transform.position, angle + 15 * j);
