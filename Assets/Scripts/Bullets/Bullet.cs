...
public class Bullet : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if(enable)
        {
            ...
            {
                var isDie = other.GetComponent<Enemy>().HitPlayerBullet(damage);
    ...
    public void Research()
    {
        ...
        foreach(GameObject t in enemies)
        {
            ...
            float distance = Vector2.Distance(t.transform.position, this.transform.position);
            if(near > distance && t.GetComponent<Enemy>().enable && Mathf.Abs(t.transform.localPosition.x) <= 4f && Mathf.Abs(t.transform.localPosition.y) <= 4.5f)
            {
                near = distance;
    ...
    private IEnumerator Split()
    {
        ...
        switch(bulletLevel[(int)BulletType.Split])
        {
            ...
            case 2:
                ...
                break;
            case 3:
                bulletGenerator.ShotPlayerBullet(this.transform.position, angle + 15, speed, damage * 0.8f, BulletType.Split);
                bulletGenerator.ShotPlayerBullet(this.transform.position, angle + 0, speed, damage * 0.8f, BulletType.Split);
                bulletGenerator.ShotPlayerBullet(this.transform.position, angle - 15, speed, damage * 0.8f, BulletType.Split);
                break;
        }
        DestroyBullet();
