...
namespace LS.Enemy
{
    public class RandomEnemy : global::Enemy.EnemyAction
    {
        private Vector3 _startPos;
        private Vector3 _entryPos;

        public RandomEnemy(Player player, EnemyBullet.EnemyBulletSpawner bulletSpawner, Vector3 startPos) : base(player, bulletSpawner)
        {
        ...
        private async UniTask Main(Transform transform, CancellationToken token)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                for (int j = 0; j <= 30; j++)
                {
                    var normalBullet = new LS.Enemy.Bullet.RandomBullet(_player, 2.5f, 1, 100, 1000);
                    float randAngle = Random.Range(0f, 360f);

                    _bulletSpawner.Spawn(normalBullet, transform.position, randAngle);
                }
                try { await UniTask.WaitForSeconds(1.25f, cancellationToken: token); }
                catch (OperationCanceledException) { return; }
            }
        }
